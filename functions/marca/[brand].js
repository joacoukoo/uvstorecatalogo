// Cloudflare Pages Function — Página de marca para SEO
// Ruta: /marca/{slug}  →  ej: uvstore.shop/marca/hot-toys
...
export async function onRequestGet(context) {
  ...
  if (!brandSlug) return Response.redirect(siteUrl, 302);
  ...
  for (const catData of Object.values(catalog)) {
    for (const p of (catData?.products || [])) {
      if (p.marca && toSlug(p.marca) === brandSlug) {
        products.push(p);
      }
    }
  }

  if (!products.length) return Response.redirect(siteUrl, 302);

  const brandName = products[0].marca;
  const siteName  = "UV Store GT";
  const title     = `Figuras ${brandName} Guatemala — ${siteName}`;
  const metaDesc  = `Comprá figuras ${brandName} en Guatemala. ${products.length} producto${products.length !== 1 ? "s" : ""} disponible${products.length !== 1 ? "s" : ""} con entrega inmediata y pre-órdenes. Consultá por WhatsApp.`;
  const canonical = `${siteUrl}/marca/${encodeURIComponent(brandSlug)}`;

  // JSON-LD ItemList
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    "name": `Figuras ${brandName} Guatemala`,
...
<h1>Figuras ${esc(brandName)} en Guatemala</h1>
<p class="subtitle">${products.length} producto${products.length !== 1 ? "s" : ""} disponible${products.length !== 1 ? "s" : ""}</p>
<div class="grid">
${cardsHtml}
</div>
<a class="wa-btn" href="https://wa.me/50230261622?text=${encodeURIComponent("Hola! Quiero consultar sobre figuras " + brandName + " en UV Store GT")}">Consultar por WhatsApp</a>
</body>
</html>`;
